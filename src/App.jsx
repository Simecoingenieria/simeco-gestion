...
function ItemsConImportar({ label, items, setItems, unidadLabel = "Und", claveUnidad = "und", claveDescripcion = ["descripcion", "material", "item", "producto"] }) {
  const lista = items || [];
...
function RequerimientosView({ data, update, clientName }) {
  ...
  return (
    <ViewShell title="Requerimientos" subtitle="Requisición del cliente (FO.GGC.02) — insumo para la cotización" action={
  ...
  return (
    <Panel title={initial ? "Editar requerimiento" : "Nuevo requerimiento"} subtitle="FO.GGC.02 — Formato de Requisición de Cotización" onClose={onClose} wide>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <ItemsConImportar label="Recurso humano" items={f.recursoHumano} setItems={(v) => set("recursoHumano", v)} unidadLabel="Días" claveUnidad="dias" claveDescripcion={["descripcion", "rol", "cargo"]} />
        </div>
        <div className="sm:col-span-2">
