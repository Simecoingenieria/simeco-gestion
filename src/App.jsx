  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F5F4F1] text-[#1B2430]">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {tab === "requerimientos" && <RequerimientosView data={data} update={update} clientName={clientName} />}
        {tab === "cotizaciones" && (
          <CotizacionesView data={data} update={update} bumpCounter={bumpCounter} clientName={clientName} />
        )}
...
/* =========================================================================
   ...
   ========================================================================= */
function RequerimientosView({ data, update, clientName }) {
  const [panel, setPanel] = useState(null);
  const save = (r) => {
    if (r.id) update("requirements", (arr) => arr.map((x) => (x.id === r.id ? r : x)));
    else update("requirements", (arr) => [...arr, { ...r, id: uid() }]);
    setPanel(null);
  };
  const remove = (id) => update("requirements", (arr) => arr.filter((r) => r.id !== id));

  return (
    <ViewShell title="Requerimientos" subtitle="Definición del alcance del servicio o proyecto por cliente" action={
      <button onClick={() => setPanel("new")} className="btnPrimary"><Plus size={14} /> Nuevo requerimiento</button>
    }>
      {data.requirements.length === 0 ? (
        <EmptyState icon={ClipboardList} text="Registra el requerimiento del cliente para definir el alcance antes de cotizar." cta="Nuevo requerimiento" onClick={() => setPanel("new")} />
      ) : (
          ...
          {data.requirements.map((r) => (
            <div key={r.id} className="rounded-xl border border-slate-200 bg-white p-4 cursor-pointer hover:border-[#C97A2B]/50" onClick={() => setPanel(r)}>
              <div className="flex items-start justify-between">
                <p className="font-medium text-slate-800">{clientName(r.clientId)}</p>
      ...
      {panel && <RequerimientoForm initial={panel === "new" ? null : panel} clients={data.clients} onSave={save} onClose={() => setPanel(null)} />}
    </ViewShell>
  );
}

function RequerimientoForm({ initial, clients, onSave, onClose }) {
  const [f, setF] = useState(initial || { clientId: clients[0]?.id || "", fecha: todayISO(), personaContacto: "", correoContacto: "", telefonoContacto: "", alcance: "", detalle: "" });
  const set = (k, v) => setF({ ...f, [k]: v });
  ...
  return (
    <Panel title={initial ? "Editar requerimiento" : "Nuevo requerimiento"} subtitle="Toda la información del cliente para definir el alcance" onClose={onClose} wide>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label="Cliente" span>
          <Select value={f.clientId} onChange={(e) => set("clientId", e.target.value)}>
            {clients.length === 0 && <option value="">Sin clientes registrados</option>}
...
/* =========================================================================
   ...
   ========================================================================= */
function CotizacionesView({ data, update, bumpCounter, clientName }) {
  const [panel, setPanel] = useState(null);
...
function CotizacionForm({ initial, clients, requirements, onSave, onClose }) {
  const [f, setF] = useState(
    initial || {
  ...
  const applyRequirement = (id) => {
    ...
    if (r) setF({ ...f, alcance: r.alcance, personaContacto: r.personaContacto, correoContacto: r.correoContacto });
  };
    ...
    <Panel title={initial ? `Editar ${initial.consecutivo}` : "Nueva cotización"} subtitle="Define alcance, costos, margen y condiciones" onClose={onClose} wide>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        {reqsForClient.length > 0 && (
          <Field label="Cargar desde requerimiento">
            <Select defaultValue="" onChange={(e) => e.target.value && applyRequirement(e.target.value)}>
              <option value="">Seleccionar…</option>
              {reqsForClient.map((r) => <option key={r.id} value={r.id}>{r.fecha} — {r.alcance.slice(0, 30)}…</option>)}
            </Select>
          </Field>
        )}
        <Field label="Persona de contacto"><TextInput value={f.personaContacto} onChange={(e) => set("personaContacto", e.target.value)} /></Field>
