...
const NAV = [
  ...
  { key: "catalogo", label: "Catálogo de Materiales", icon: Package },
...
function LineItems({ label, items, setItems }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {items.length > 0 && (
        <div className="mb-1 grid grid-cols-12 gap-1.5 px-0.5">
          <span className="col-span-4 text-[10px] font-medium uppercase text-slate-400">Descripción</span>
          <span className="col-span-2 text-[10px] font-medium uppercase text-slate-400">Cant</span>
          <span className="col-span-1 text-[10px] font-medium uppercase text-slate-400">Und</span>
          <span className="col-span-2 text-[10px] font-medium uppercase text-slate-400">V. Unit</span>
          <span className="col-span-2 text-[10px] font-medium uppercase text-slate-400 text-right">Total</span>
        </div>
      )}
        ...
        {items.map((r, i) => (
          <div key={i} className="grid grid-cols-12 gap-1.5 items-center">
            <input
              ...
              className="col-span-4 rounded border border-slate-200 px-2 py-1 text-xs"
            />
            <input
              type="number"
              placeholder="Cant"
            ...
            <input
              ...
              className="col-span-1 rounded border border-slate-200 px-2 py-1 text-xs"
            />
            <input
              type="number"
              placeholder="V. Unit"
              ...
              className="col-span-2 rounded border border-slate-200 px-2 py-1 text-xs"
            />
            <span className="col-span-2 text-right text-xs font-medium text-slate-700 truncate">
              {cop((Number(r.cant) || 0) * (Number(r.valorUnit) || 0))}
            </span>
            <button type="button" onClick={() => setItems(items.filter((_, idx) => idx !== i))} className="col-span-1 flex justify-end text-slate-300 hover:text-rose-500">
              <X size={14} />
            </button>
          </div>
        ))}
...
/* --------------- MATERIALES CON AUTOCOMPLETADO DE PRECIO ---------------- */
// Igual que LineItems, pero la descripción se busca contra el catálogo
// (catalogo_materiales en Supabase) y, al encontrar una coincidencia
// exacta, autocompleta unidad y precio unitario automáticamente.
function LineItemsMateriales({ label, items, setItems, catalogo }) {
  const total = items.reduce((s, r) => s + (Number(r.cant) || 0) * (Number(r.valorUnit) || 0), 0);
  const upd = (i, field, val) => {
    let copy = items.map((r, idx) => (idx === i ? { ...r, [field]: val } : r));
    if (field === "desc") {
      const match = (catalogo || []).find(
        (m) => m.descripcion.toLowerCase() === val.toLowerCase() || m.codigo?.toLowerCase() === val.toLowerCase()
      );
      if (match) {
        copy = copy.map((r, idx) =>
          idx === i ? { ...r, desc: match.descripcion, und: match.unidad || r.und, valorUnit: match.precio } : r
        );
      }
    }
    setItems(copy);
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {catalogo && catalogo.length > 0 && (
            <span className="text-[10px] text-emerald-600">{catalogo.length} en catálogo — autocompleta precio</span>
          )}
          <button
            type="button"
            onClick={() => setItems([...items, { desc: "", cant: 1, und: "und", valorUnit: 0 }])}
      ...
      <datalist id="catalogo-materiales-datalist">
        {(catalogo || []).map((m) => (
          <option key={m.id} value={m.descripcion}>{m.codigo ? `${m.codigo} — ${cop(m.precio)}` : cop(m.precio)}</option>
        ))}
      </datalist>
      {items.length === 0 && <p className="py-2 text-xs text-slate-400">Sin ítems.</p>}
      {items.length > 0 && (
      ...
      <div className="space-y-1.5">
        {items.map((r, i) => (
          <div key={i} className="grid grid-cols-12 gap-1.5 items-center">
            <input
              placeholder="Descripción (busca en el catálogo)"
              value={r.desc}
              onChange={(e) => upd(i, "desc", e.target.value)}
              list="catalogo-materiales-datalist"
              className="col-span-4 rounded border border-slate-200 px-2 py-1 text-xs"
            />
            <input
              type="number"
...
export default function App() {
  ...
  const reloadCatalogo = useCallback(async () => {
    ...
    if (!error && rows) setCatalogo(rows);
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F5F4F1] text-[#1B2430]">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {tab === "catalogo" && (
          <CatalogoView catalogo={catalogo} reloadCatalogo={reloadCatalogo} loadingCatalogo={loadingCatalogo} />
        )}
        {tab === "cotizaciones" && (
          <CotizacionesView
            data={data}
...
/* =========================================================================
   ...
   ========================================================================= */
/* =========================================================================
   CATÁLOGO DE MATERIALES — precios reales de proveedores, mantenidos
   manualmente. Alimenta el autocompletado en Materiales de Cotizaciones.
   ========================================================================= */
function CatalogoView({ catalogo, reloadCatalogo, loadingCatalogo }) {
  const [panel, setPanel] = useState(null);
  const [q, setQ] = useState("");
  const [guardando, setGuardando] = useState(false);
  const [importando, setImportando] = useState(false);
  ...
  const filtered = catalogo.filter((m) =>
    (m.descripcion + " " + (m.codigo || "") + " " + (m.proveedor || "")).toLowerCase().includes(q.toLowerCase())
  );

  const save = async (m) => {
    setGuardando(true);
    const fila = { codigo: m.codigo || null, descripcion: m.descripcion, unidad: m.unidad || "und", precio: Number(m.precio) || 0, proveedor: m.proveedor || null, categoria: m.categoria || null };
    let error;
    if (m.id) ({ error } = await supabase.from("catalogo_materiales").update(fila).eq("id", m.id));
    else ({ error } = await supabase.from("catalogo_materiales").insert(fila));
    setGuardando(false);
  ...
  const importarExcel = async (e) => {
    ...
    try {
      ...
      const hoja = libro.Sheets[libro.SheetNames[0]];
      const filas = XLSX.utils.sheet_to_json(hoja, { defval: "" });
      if (!filas.length) { setErrorImport("El archivo no tiene filas legibles."); return; }
      const nuevos = filas
        .map((fila) => ({
          codigo: String(encontrarColumnaExcel(fila, ["codigo", "código", "sku"])) || null,
          descripcion: String(encontrarColumnaExcel(fila, ["descripcion", "descripción", "material", "item", "producto"])),
          unidad: String(encontrarColumnaExcel(fila, ["unidad", "und", "um"])) || "und",
          precio: Number(encontrarColumnaExcel(fila, ["precio", "valor", "costo"])) || 0,
          proveedor: String(encontrarColumnaExcel(fila, ["proveedor", "marca"])) || null,
        }))
        .filter((m) => m.descripcion.trim() !== "");
      if (!nuevos.length) { setErrorImport("No se encontraron columnas de descripción/precio reconocibles."); return; }
      const { error } = await supabase.from("catalogo_materiales").insert(nuevos);
      if (error) { setErrorImport("Error al guardar en Supabase: " + error.message); return; }
      await reloadCatalogo();
    } catch (err) {
      setErrorImport("No se pudo leer el archivo. Confirma que sea .xlsx o .csv.");
    } finally {
      setImportando(false);
      e.target.value = "";
    }
  ...
  return (
    <ViewShell
      ...
      subtitle="Precios reales de tus proveedores — alimenta el autocompletado en Cotizaciones"
      action={
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setPanel("new")} className="btnPrimary"><Plus size={14} /> Nuevo material</button>
        </div>
      }
    >
      {errorImport && <div className="mb-4 rounded-md bg-rose-50 border border-rose-200 px-3 py-2 text-xs text-rose-600">{errorImport}</div>}
      <div className="mb-4 relative max-w-md">
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por descripción, código o proveedor…" className={inputCls + " pl-8"} />
      </div>

      {loadingCatalogo ? (
        <p className="py-10 text-center text-sm text-slate-400">Cargando catálogo desde Supabase…</p>
      ) : filtered.length === 0 ? (
        <EmptyState icon={Package} text="Agrega materiales con su precio real, o impórtalos desde un Excel, para que Cotizaciones los autocomplete." cta="Nuevo material" onClick={() => setPanel("new")} />
      ) : (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <table className="w-full">
            <thead className="border-b border-slate-200 bg-slate-50"><tr><Th>Código</Th><Th>Descripción</Th><Th>Und</Th><Th>Precio</Th><Th>Proveedor</Th><Th></Th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {filtered.map((m) => (
                <tr key={m.id} className="hover:bg-slate-50/60">
                  ...
                  <Td>{cop(m.precio)}</Td>
                  <Td>{m.proveedor || "—"}</Td>
                  <Td><RowActions onEdit={() => setPanel(m)} onDelete={() => remove(m.id)} /></Td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {panel && <CatalogoForm initial={panel === "new" ? null : panel} onSave={save} onClose={() => setPanel(null)} guardando={guardando} />}
    </ViewShell>
  );
}

function CatalogoForm({ initial, onSave, onClose, guardando }) {
  const [f, setF] = useState(initial || { codigo: "", descripcion: "", unidad: "und", precio: 0, proveedor: "", categoria: "" });
  const set = (k, v) => setF({ ...f, [k]: v });
  return (
    <Panel title={initial ? "Editar material" : "Nuevo material"} onClose={onClose}>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label="Código"><TextInput value={f.codigo} onChange={(e) => set("codigo", e.target.value)} placeholder="Ej. CB12B" /></Field>
        <Field label="Unidad"><TextInput value={f.unidad} onChange={(e) => set("unidad", e.target.value)} placeholder="und, m, kg…" /></Field>
        <Field label="Descripción" span><TextInput value={f.descripcion} onChange={(e) => set("descripcion", e.target.value)} placeholder="Ej. Cable THHN 12 AWG" /></Field>
        <Field label="Precio (COP)"><TextInput type="number" value={f.precio} onChange={(e) => set("precio", e.target.value)} /></Field>
        <Field label="Proveedor"><TextInput value={f.proveedor} onChange={(e) => set("proveedor", e.target.value)} placeholder="Interelectricas, Ineldec…" /></Field>
      </div>
      <div className="mt-5 flex justify-end gap-2">
        <button onClick={onClose} className="btnGhost">Cancelar</button>
        <button onClick={() => onSave(f)} disabled={guardando} className="btnPrimary" style={{ opacity: guardando ? 0.6 : 1 }}>
...
function CotizacionesView({ data, update, bumpCounter, clientName, prefillReqId, clearPrefillReqId, catalogo }) {
  const [panel, setPanel] = useState(null);
...
function CotizacionForm({ initial, clients, requirements, prefillReqId, catalogo, onSave, onClose }) {
  const [f, setF] = useState(() => {
  ...
  return (
    <Panel title={initial ? `Editar ${initial.consecutivo}` : "Nueva cotización"} subtitle="Define alcance, costos, margen y condiciones" onClose={onClose} wide>
      ...
      <div className="mt-4 space-y-3">
        <LineItemsMateriales label="Materiales eléctricos" items={f.materiales} setItems={(v) => set("materiales", v)} catalogo={catalogo} />
